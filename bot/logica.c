...
ARVORE valor_jogada_recursiva (ARVORE tree){
    double x;
    if(tree != NULL) {
        if(tree->passagens==0 && tree->valor!=0){
            tree->passagens++;
        }
        else{
            x= tree->valor;
            tree->valor = valor_jogada(tree) + x;
            if(x!=tree->valor)
                tree->passagens++;
        }
...
/*double valor_jogada (ARVORE tree) {
    ...
    if(tree->SM != NULL) vsm = tree->SM->valor;
    if(tree->SD != NULL) vsd = tree->SD->valor;
    if(tree->ME != NULL) vme = tree->ME->valor;
    if(tree->MD != NULL) vmd = tree->MD->valor;
    if(tree->IE != NULL) vie = tree->IE->valor;
    if(tree->IM != NULL) vim = tree->IM->valor;
    if(tree->ID != NULL) vid = tree->ID->valor;
